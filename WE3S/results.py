...
class Results():
    def __init__(self, record_data):
        ...
        self.doze_to_awake_STA = []
        self.doze_to_awake_avg = -1
        self.doze_to_awake_std = -1
    ...
    def compute_counters(self):
        if len(self.idle_STA) == 0 or len(self.CCA_STA) == 0 or len(self.Rx_STA) == 0 or len(self.Tx_STA) == 0 or len(self.doze_STA) == 0:
            ...
            self.doze_to_awake_STA = [0 for i in range(self.nb_STAs)]
            self.awake_to_doze_STA = [0 for i in range(self.nb_STAs)]
            for sta in self.record_data["Events"][self.last_key]["STAs"]:
                if sta != "AP":
                    STA_ID = int(sta) - 1 # Minus 1 to get the proper index
                    ...
                    switch_to_awake = self.record_data["STAs"][sta]["counters"]["Switch doze to awake"]
                    switch_to_doze = self.record_data["STAs"][sta]["counters"]["Switch awake to doze"]
                    self.awake_to_doze_STA[STA_ID] = switch_to_doze * float(SWITCH_AWAKE_TO_DOZE_DURATION)
                    self.doze_to_awake_STA[STA_ID] = switch_to_awake * float(SWITCH_DOZE_TO_AWAKE_DURATION)
                    self.doze_STA[STA_ID] += self.record_data["Events"][self.last_key]["STAs"][sta]["doze time"]
                    self.doze_STA[STA_ID] -= self.awake_to_doze_STA[STA_ID]
                    self.doze_STA[STA_ID] -= self.doze_to_awake_STA[STA_ID]
            ...
            for sta in range(self.nb_STAs):
                ...
                self.doze_to_awake_STA[sta] /= self.simulation_duration
                self.awake_to_doze_STA[sta] /= self.simulation_duration
            ...
            self.doze_to_awake_avg = sum(self.doze_to_awake_STA) / self.nb_STAs
            self.awake_to_doze_avg = sum(self.awake_to_doze_STA) / self.nb_STAs
            ...
            self.doze_to_awake_std = sum([(x - self.doze_to_awake_avg)**2 for x in self.doze_to_awake_STA])
            self.awake_to_doze_std = sum([(x - self.awake_to_doze_avg)**2 for x in self.awake_to_doze_STA])
            ...
            self.doze_to_awake_std = (self.doze_to_awake_std / self.nb_STAs) **.5
            self.awake_to_doze_std = (self.awake_to_doze_std / self.nb_STAs) **.5
    ...
    def compute_energy_consumption(self):
        if len(self.consumption_STA) == 0:
            ...
            for sta in range(self.nb_STAs):
                ...
                self.consumption_STA[sta] += self.doze_to_awake_STA[sta] * SWITCH_DOZE_TO_AWAKE_CONSUMPTION
                self.consumption_STA[sta] += self.awake_to_doze_STA[sta] * SWITCH_AWAKE_TO_DOZE_CONSUMPTION
                ...
                # if real_doze_time < 0:
                #     print("Real doze time:", real_doze_time)
                #     print("Total doze time:", self.doze_STA[sta])
                #     print("Awake to doze:", awake_to_doze_time)
                #     print("Doze to awake:", doze_to_awake_time)
                #     print("nb switchs awake to doze:", self.nb_switch_awake_to_doze_STA[sta])
                #     print("nb switchs doze to awake:", self.nb_switch_doze_to_awake_STA[sta])
    ...
    def get_dictionary(self):
        ...
        doze_to_awake = dict()
        for x,y in enumerate(self.doze_to_awake_STA):
            doze_to_awake[str(x+1)] = y
        result["doze to awake STA"] = doze_to_awake
        result["doze to awake avg"] = self.doze_to_awake_avg
        result["doze to awake std"] = self.doze_to_awake_std

        awake_to_doze = dict()
        for x,y in enumerate(self.awake_to_doze_STA):
            awake_to_doze[str(x+1)] = y
        result["awake to doze STA"] = awake_to_doze
        result["awake to doze avg"] = self.awake_to_doze_avg
        result["awake to doze std"] = self.awake_to_doze_std
